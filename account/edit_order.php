...
// Получаем данные заказа для редактирования
if (isset($_GET['id'])) {
    $order_id = $_GET['id'];
    $sql = "SELECT * FROM orders WHERE id_orders = $order_id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE orders SET dataStart = '$dataStart', dataEnd = '$dataEnd', status = '$status', stars = ?, comment = ?, id_project = '$id_project', id_user = '$id_user', id_designer = '$id_designer' WHERE id_orders = $order_id";
...
<header>
    ...
    <main>
        <form method="post" action="">
            ...
            <input type="date" id="dataStart" name="dataStart" value="<?php echo $order['dataStart']; ?>" required>

            <label for="dataEnd">Дата завершения:</label>
            <input type="date" id="dataEnd" name="dataEnd" value="<?php echo $order['dataEnd']; ?>" required>

            <label for="status">Статус:</label>
            <input type="text" id="status" name="status" value="<?php echo $order['status']; ?>" required>

            <label for="stars">Оценка:</label>
            <input type="number" id="stars" name="stars" min="0" max="5" value="<?php echo $order['stars']; ?>" >

            <label for="comment">Комментарий:</label>
            <textarea id="comment" name="comment" required><?php echo $order['comment']; ?></textarea>

            <label for="id_project">ID проекта:</label>
            <input type="number" id="id_project" name="id_project" value="<?php echo $order['id_project']; ?>" required>
            ...
            <input type="number" id="id_designer" name="id_designer" value="<?php echo $order['id_designer']; ?>" required>
